...
function TabbedObjectFieldTemplate(props) {
    const getTabsProperties = function getTabsProperties() {
        ...
        props.properties.map(prop => {
            ...
            if (!(targetTab in tabsProperties))
                tabsProperties[targetTab] = [];
    ...
    return (
        <fieldset>
            ...
            <Well>
                <Tab.Container id={`${props.idSchema.$id}__tabs`} defaultActiveKey={Object.keys(tabsProperties)[0]}>
                  <Row className="clearfix">
                    <Col sm={2}>
                      <Nav bsStyle="pills" stacked>
                        {Object.keys(tabsProperties).map(tabName => {return (
