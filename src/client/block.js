...
export class Block extends React.Component {
    ...
    componentDidMount() {
        if (this.props.block != null) {
            ...
            list_all_parts().then(parts => { this.setState({ partsList: parts }) }
            );

            list_all_coders().then(coders => { this.setState({ codersList: coders }) }
            );

            list_all_blocks().then(blocks => { this.setState({ blocksList: blocks }) }
            );
