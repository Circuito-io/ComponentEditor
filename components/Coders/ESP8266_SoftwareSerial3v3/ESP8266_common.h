...
class ESP8266 {
	...
	#ifdef ESP8266_USE_SOFTWARE_SERIAL
	/*
	* Constuctor for SoftwareSerial case.
	*/
	ESP8266(int pinRx, int pinTx);
	#else /* HardwareSerial */
	/*
	* Constuctor for HardwareSerial case.
	*/
	ESP8266();
	#endif
	...
	*/
	bool init(const char* ssid, const char* pwd, uint32_t baudRateSet = 9600);
	bool initSequence(const char* ssid, const char* pwd, uint32_t baudRateSet = 9600);
	...
	*/
	...
	int recvSingle(char *buffer, int maxLen);
	bool sendSingle(const char* body, const char* header, const char* footer);
	int httpGet(const char* host, int hostPort);
  char* getBuffer();
/*
	void setDweet(const char* key, const char* value);
	void sendDweetKeys();
	void receiveDweetEvents();
	int getValue(char* str, const char* key);
*/
  ...
  char* const m_urlHeader = "GET / HTTP/1.1\r\nHost: ";
  char* const m_urlFooter = "\r\nUser-Agent: ESP8266-WiFi/1.0\r\nConnection: close\r\n\r\n";
