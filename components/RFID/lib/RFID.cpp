...
String RFID::readTag()
{
  ...
  if(isCard())
  {
      //if card is present print it to serial monitor
      if(readCardSerial())
      {
        str = getTag();
        }
...
/*
 * get tag returns a string containing the current tag  discovered
 */
String RFID::getTag()
{
  String str = "";
...
/*
 * printTag will print the the tag only if it is not "None"
 */
void RFID::printTag(String tag)
{
  if(tag != "None")
  {
